...
- 比較用 LLM()（L1 / L2）は同じホストの Chat Completions。モデルは 1 つに固定し、
  実応答のバージョンをログに残す。
...
実行単位が「ケースごとに 7 条件を連続実行」であるため、課題ごとに独立した実行系を
持つと条件間比較の同時性が崩れる。

| コンポーネント | 責務 | 課題固有か |
| --- | --- | --- |
| ケース台帳 | 配布元から取得、100 件抽出、dev/test 分割、ID と本文ハッシュの永続化 | 抽出ロジックのみ固有 |
| プロンプトカタログ | 条件 A〜C / L1 / L2 の questions または LLM テンプレート | 中身は固有、読み出しは共通 |
...
flowchart TD
  ...
  subgraph 実行["実行（以降は data/ のみ）"]
    ...
    D[dev: A の作り込み 最大3回]
    G[generate: B1-B3 / C / L1 / L2 を1軸だけ変える]
    T[test: 60件 × 7条件 × 3課題]
    M[metrics + stats]
    R[公開用 results]
    P --> D --> G --> T --> M --> R
...
見積もりリクエスト数は設計書どおり、本ラン 2,100（Jev 1,500 / LLM 600）、
LLM ばらつき測定で +2,400。fan-out はこれに含めない。
...
- L1: タスク 1 文 + ラベル一覧 + 「ラベル名だけ返せ」
- L2: 役割、A の criteria の散文化、混同ペア、JSON スキーマ、few-shot 3 件
